  ...
  if (isWriting) {
    return (
      <div className="flex flex-col h-full relative text-white animate-in fade-in duration-200">
        ...
        <button className="mt-4 bg-primary-100 text-white py-3 rounded-lg font-bold hover:bg-primary-200 transition-colors">
          저장하기
        </button>
      </div>
    );
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-3 shrink-0">
        ...
        <button className="text-primary-100 font-medium text-text-3 hover:text-primary-200">
          편집
        </button>
      </div>
      ...
      <div className="overflow-y-auto h-full pb-10 scrollbar-hide w-full mx-auto">
        <div className="grid grid-cols-2 gap-3">
          <NoteCard
            title="전공 수업"
            date="1일 전"
            preview="탄소강의 열처리 과정에 대해..."
          />
          <NoteCard
            title="캡스톤 회의"
            date="2일 전"
            preview="다음 주 발표 자료 준비..."
          />
          <NoteCard
            title="아이디어"
            date="3일 전"
            preview="기계 팔 관절 설계 변경안..."
          />
          <NoteCard
            title="할일"
            date="4일 전"
